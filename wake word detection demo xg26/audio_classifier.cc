...
static void handle_result(int32_t current_time, int result, uint8_t score, bool is_new_command)
{
  ...
  // 2 = silence
  //
  // Chỉ class 0 = hey_snips mới được bật DETECTION_LED.
  if (is_new_command && result == 0) {
    ...
    // unknown/silence thì bỏ qua, chỉ log để debug
    printf("Ignored class=%d label=%s score=%d @%ldms\n",
           result, label, score, current_time);
  ...
  // Khi detection timeout đã hết, kiểm tra activity/noise để nháy ACTIVITY_LED
